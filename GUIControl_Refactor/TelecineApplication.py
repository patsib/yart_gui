...
# collect all local settings from GUI and store to sel
...
# Get all camera settings
    def getCameraSettings(self):
        ...
        settings = self.sock.receiveObject()
        # print(settings)
        #redo for correct red/blue gain settings (hack to fix latency without refactoring raspberry controller)
        self.sock.sendObject((GET_CAMERA_SETTINGS,))
        settings = self.sock.receiveObject()
        # print(settings)
...
# PSI: modified for multiple connection attempts and feedback
...
# PSI: modified: try for the socket.connect / timeout 5sec
    def connect(self):
        socke = socket.socket()
        socke.settimeout(5)
        self.ip_pi = self.ipLineEdit.text()
